...
def fetch_models(provider, api_key):
    try:
        response = requests.get(_models_url(provider), headers=_headers(provider, api_key), timeout=45)
        if response.status_code < 200 or response.status_code >= 300:
            return json.dumps({"ok": False, "error": f"HTTP {response.status_code}: {response.text}"}, ensure_ascii=False)
        data = response.json()
        models = []
...
def chat(provider, model, api_key, message, history_json="[]", attachment_name=None, attachment_mime=None, attachment_b64=None):
    try:
        ...
        response = requests.post(_chat_url(provider), headers=_headers(provider, api_key), json=payload, timeout=TIMEOUT)
        if response.status_code < 200 or response.status_code >= 300:
            return json.dumps({"ok": False, "error": f"HTTP {response.status_code}: {response.text}"}, ensure_ascii=False)
        content = _parse_chat_response(response.json())
...
def generate_image(provider, api_key, prompt):
    try:
        ...
        response = requests.post("https://api.openai.com/v1/images/generations", headers=_headers("openai", api_key), json=payload, timeout=120)
        if response.status_code < 200 or response.status_code >= 300:
            return json.dumps({"ok": False, "error": f"HTTP {response.status_code}: {response.text}"}, ensure_ascii=False)
        data = response.json()
        if data.get("error"):
